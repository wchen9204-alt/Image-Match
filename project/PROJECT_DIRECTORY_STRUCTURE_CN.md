...
## 一致性结�?
旧版 `PROJECT_DIRECTORY_STRUCTURE_CN.md` 已经和当前项目不完全一致：顶层顺序没有按项目目录展示，`configs/` 位置偏后；同时缺�?`build/`、`SOURCE_FILE_DOCS_CN/`、`CODEX_WORKING_RULES_CN.md`、直接法/学习法相关新增目录、`contour_descriptor`、`string_utils`、`direct_data`、`Test11`、`Test12` 等当前文件或目录�?
下面按当�?`project/` 目录的实际顺序整理。构建目录、输出目录和第三方库只列关键层级，避免把大量生成物和外部依赖文件展开到文档里�?
```
project/                                     # 图像配准实验平台根目�?├── apps/                                    # 应用程序入口
�?  ├── registration_app.cpp                 # CLI 解析、单�?批量运行、结果输�?�?  └── registration_app.h                   # RegistrationApp 类声�?�?├── build/                                   # CMake/IDE 构建目录（生成物，非源码�?�?  └── CMakeFiles/                          # CMake 中间文件
�?├── build-mingw/                             # MinGW 构建与运行目录（生成物）
�?  ├── bin/                                 # 可执行文件与运行时配置副�?�?  ├── CMakeFiles/                          # CMake 中间文件
�?  └── ...                                  # 其他构建缓存和目标文�?�?├── configs/                                 # YAML 配置文件
�?  ├── direct/                              # 直接法配�?�?  �?  ├── dis_flow.yaml
�?  �?  ├── ecc.yaml
�?  �?  ├── esm_rigid.yaml
�?  �?  ├── farneback.yaml
�?  �?  ├── fourier_mellin.yaml
...
�?  �?  ├── benchmark.yaml
�?  �?  └── metrics.yaml
�?  ├── filter/                              # 匹配过滤器配�?�?  �?  ├── cross_check.yaml
�?  �?  ├── distance_distribution.yaml
�?  �?  ├── distance_threshold.yaml
�?  �?  ├── gms.yaml
�?  �?  ├── min_distance.yaml
�?  �?  └── ratio_test.yaml
�?  ├── geometry/                            # 几何估计器配�?�?  �?  ├── affine.yaml
�?  �?  ├── homography.yaml
�?  �?  ├── rigid.yaml
�?  �?  └── similarity.yaml
�?  ├── keypoint/                            # 点特征提取器配置
...
�?  �?  ├── sift.yaml
�?  �?  └── surf.yaml
�?  ├── learning/                            # 深度学习匹配器配�?�?  �?  ├── loftr.yaml
�?  �?  ├── superpoint_lightglue.yaml
�?  �?  └── superpoint_superglue.yaml
�?  ├── matcher/                             # 点特征匹配器配置
�?  �?  ├── bf.yaml
�?  �?  └── flann.yaml
�?  ├── pipeline/                            # 流水线编排配�?�?  �?  ├── batch/                           # 批量运行配置
�?  �?  �?  ├── batch_direct.yaml
�?  �?  �?  ├── batch_keypoint.yaml
�?  �?  �?  ├── batch_learning.yaml
...
�?  �?  �?  ├── sift_pipeline.yaml
�?  �?  �?  └── surf_pipeline.yaml
�?  �?  ├── learning/                        # 深度学习流水�?�?  �?  �?  ├── loftr_learning_pipeline.yaml
�?  �?  �?  ├── superpoint_lightglue_learning_pipeline.yaml
�?  �?  �?  └── superpoint_superglue_learning_pipeline.yaml
�?  �?  └── structure/                       # 结构特征流水�?�?  �?      ├── contour_pipeline.yaml
�?  �?      ├── edge_pipeline.yaml
�?  �?      └── line_pipeline.yaml
�?  └── structure/                           # 结构特征提取与关联配�?�?      ├── contour.yaml
�?      ├── edge.yaml
�?      └── line.yaml
�?├── datasets/                                # 测试数据�?�?  ├── Test01/                              # source.png + target.png
...
�?├── include/                                 # 公共头文�?�?  ├── core/                                # 核心基础设施
�?  �?  ├── config.h                         # 配置加载与路径解析：定义 MethodFamily、PipelineConfig �?Config::load/loadPipeline/resolvePath
�?  �?  ├── context.h                        # RegistrationContext 上下文：贯穿单次配准流程，集中保存图像、特征、匹配、几何、评测和输出状�?�?  �?  ├── factory.h                        # Factory 静态工厂：根据 YAML 枚举和参数创建提取器、匹配器、过滤器、估计器、直接法和流水线对象
�?  �?  ├── registration.h                   # Registration 顶层门面：实�?IRegistration，负责配置流水线并对外提供统一 run 入口
�?  �?  ├── result.h                         # RegistrationResult 结果摘要：记录成功状态、失败原因、匹�?内点数量、耗时、IoU、NMAD 等运行指�?�?  �?  └── types.h                          # 全局枚举与转换工具：KeypointType、StructureType、NormType、MatchMethod、GeometryType、TransformType、ImageIndex
�?  ├── data/                                # 流水线数据结�?�?  �?  ├── correspondence_view.h            # CorrespondenceView 统一对应点视图：把点特征、结构、直接法、学习法结果转换成通用点对/匹配/内点只读接口
...
�?  �?  ├── feature_initializer_data.h       # FeatureInitializerData 直接法前置点特征初值：记录候选方法、接收状态、几何质量和临时 warp 质量
�?  �?  ├── geometry_data.h                  # GeometryData 几何估计结果：保存模型类型、H/A 变换矩阵、内点掩码、内点数、重投影误差和有效�?�?  �?  ├── image_data.h                     # ImagePairData 图像对缓存：保存 source/target �?BGR 图、灰度图和输入路径，供各流水线共�?�?  �?  ├── keypoint_data.h                  # KeypointData 点特征数据：分别保存两幅图的关键点、描述子、提取器名称和特征数�?�?  �?  ├── keypoint_match_data.h            # KeypointMatchData 点匹配数据：保存原始匹配、过滤后匹配、内点匹配以及与几何估计对齐的索引关�?�?  �?  ├── structure_data.h                 # StructureData 结构特征数据：保存边�?线段/轮廓响应图、结构基元、描述子和提取方法信�?�?  �?  ├── structure_match_data.h           # StructureMatchData 结构关联结果：保存结构点对、线段匹配、估计平移、仿射初值、得分和关联有效�?�?  �?  └── transform_data.h                 # TransformData 最终变换数据：保存可用�?warping 的矩阵、变换类型、图像尺寸、有效性和说明信息
�?  ├── dataset/                             # 数据集加�?�?  �?  ├── dataset_loader.h
�?  �?  └── sample.h
�?  ├── direct/                              # 直接法配准器
�?  �?  ├── common/
�?  �?  �?  ├── dense_flow_common.h
...
�?  ├── matcher/                             # 匹配器与结构关联�?�?  �?  ├── feature/                         # 预留特征匹配目录
�?  �?  ├── keypoint/
�?  �?  �?  ├── bf_matcher.h
�?  �?  �?  └── flann_matcher.h
�?  �?  └── structure/
...
�?  ├── structure/                           # 结构特征提取�?�?  �?  ├── contour_extractor.h
�?  �?  ├── edge_extractor.h
�?  �?  └── line_extractor.h
...
�?├── outputs/                                 # 运行输出目录（生成物�?�?  ├── batch/                               # 批量运行结果
�?  ├── compare/                             # 横向对比实验结果
�?  ├── debug_ecc_fix/                       # ECC 调试输出
�?  └── single/                              # 单次运行结果
�?├── SOURCE_FILE_DOCS_CN/                     # 源文件中文说明文�?�?  ├── CORE_CONFIG_FUNCTIONS_CN.md
...
�?  �?  ├── evaluator.cpp
�?  �?  ├── metrics/
�?  �?  �?  ├── feature/                     # 当前无实现文�?�?  �?  �?  ├── geometric/
�?  �?  �?  �?  └── inlier_ratio.cpp
�?  �?  �?  ├── image/                       # 当前无实现文�?�?  �?  �?  └── keypoint/
�?  �?  �?      └── repeatability.cpp
�?  �?  └── quality/
...
�?├── third_party/                             # 第三方源码依�?�?  ├── LightGlue/                           # LightGlue Python 依赖
...
�?├── .clang-format                            # C/C++ 格式化配�?├── CMakeLists.txt                           # CMake 构建入口
├── CODEX_WORKING_RULES_CN.md                # Codex 协作/工作规则
├── main.cpp                                 # 程序入口
├── PROJECT_DIRECTORY_STRUCTURE_CN.md        # 当前目录结构说明
├── PROJECT_PIPELINE_INTERNALS_CN.md         # 平台内部流水线、数据流、Factory �?YAML 机制说明
├── PROJECT_QUICK_INTRO_CN.md                # 中文快速入�?└── README.md                                # 项目说明
```
