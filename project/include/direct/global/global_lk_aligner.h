...
/// 全局 Lucas-Kanade 刚体直接法配准器。
/// 该方法只优化旋转和平移三个自由度，在同步下采样的灰度图金字塔上最小化全局 SSD 光度残差。
class GlobalLkAligner : public IDirectAligner {
public:
    explicit GlobalLkAligner(const YAML::Node& cfg);

    std::string name() const override { return "GLOBAL_LK"; }
    bool align(RegistrationContext& ctx) override;

private:
    /// 每层金字塔的最大 Gauss-Newton 迭代次数。
