...
/// 从 OpenCV partial affine 的 sR 中去掉统一缩放，只保留旋转，再基于当前内点重算平移。
/// 该模式比严格 SVD 回归更接近“先用 similarity 初值，再压回 rigid”。
inline bool refineRigidByNormalizeScaleFromMask(const std::vector<cv::Point2f>& src,
                                                const std::vector<cv::Point2f>& dst,
                                                double threshold,
                                                std::vector<unsigned char>& mask,
                                                cv::Mat& A,
    ...
    const double a = A64.at<double>(0, 0);
    const double b = A64.at<double>(0, 1);
    const double c = A64.at<double>(1, 0);
    const double d = A64.at<double>(1, 1);
    const double sx = std::sqrt(a * a + c * c);
    const double sy = std::sqrt(b * b + d * d);
    removedScale = 0.5 * (sx + sy);
    if (removedScale <= 1e-9) {
        return false;
    }

    cv::Mat R = (cv::Mat_<double>(2, 2) << a / removedScale,
                 b / removedScale,
                 c / removedScale,
                 d / removedScale);

    // 再做一次正交化，避免数值误差把旋转矩阵带偏。
    ...
    auto buildRigidWithRotation = [](const std::vector<cv::Point2f>& inSrc,
                                     const std::vector<cv::Point2f>& inDst,
                                     const cv::Mat& rotation,
                                     cv::Mat& outA) {
        if (inSrc.size() != inDst.size() || inSrc.size() < 2) {
            return false;
        }

        cv::Point2d c1(0.0, 0.0);
        cv::Point2d c2(0.0, 0.0);
        for (size_t i = 0; i < inSrc.size(); ++i) {
            c1.x += inSrc[i].x;
            c1.y += inSrc[i].y;
            c2.x += inDst[i].x;
            c2.y += inDst[i].y;
        }
        c1.x /= static_cast<double>(inSrc.size());
        c1.y /= static_cast<double>(inSrc.size());
        c2.x /= static_cast<double>(inDst.size());
        c2.y /= static_cast<double>(inDst.size());

        const cv::Mat c1m = (cv::Mat_<double>(2, 1) << c1.x, c1.y);
        const cv::Mat c2m = (cv::Mat_<double>(2, 1) << c2.x, c2.y);
        const cv::Mat t = c2m - rotation * c1m;

        outA = cv::Mat::zeros(2, 3, CV_64F);
        rotation.copyTo(outA(cv::Rect(0, 0, 2, 2)));
        outA.at<double>(0, 2) = t.at<double>(0, 0);
        outA.at<double>(1, 2) = t.at<double>(1, 0);
        return true;
    };
    ...
    if (!buildRigidWithRotation(inlierSrc, inlierDst, R, A)) {
        return false;
    }

    mask = maskByReprojection(src, dst, A, threshold);
    collectMaskedPoints(src, dst, mask, inlierSrc, inlierDst);
    if (inlierSrc.size() >= 2 && buildRigidWithRotation(inlierSrc, inlierDst, R, A)) {
        mask = maskByReprojection(src, dst, A, threshold);
    }
