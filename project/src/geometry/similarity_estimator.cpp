...
SimilarityEstimator::SimilarityEstimator(const YAML::Node& cfg) {
    ...
    IR_LOG_INFO("SimilarityEstimator: method=", method_str,
...
bool SimilarityEstimator::estimate(RegistrationContext& ctx) {
    ...
    if (md.filtered.size() < 2) {
        ...
        IR_LOG_ERROR("SimilarityEstimator: need at least 2 matches, got ",
                     md.filtered.size());
        return false;
    }
    ...
    if (!gd.valid) {
        gd.message = partial_affine_utils::rejectMessage(
            "similarity transform", inliers, _minInliers);
        IR_LOG_WARN("SimilarityEstimator rejected model: ", gd.message);
    }

    IR_LOG_INFO("Similarity2D inliers=", inliers, " / ", md.filtered.size(),
