param(
    ...
    [string]$ReportName = 'KEYPOINT_COMPARE_TEST_REPORT_CN',
    [switch]$SkipPdf
)
...
$CompareRoot = Join-Path $ProjectRoot 'outputs\compare\keypoint'
$HtmlPath = Join-Path $ProjectRoot "$ReportName.html"
$PdfPath = Join-Path $ProjectRoot "$ReportName.pdf"
$OutputReportDir = Join-Path $ProjectRoot 'reports\keypoint'

$methods = @(
    [pscustomobject]@{ Label='akaze'; Display='AKAZE' },
    [pscustomobject]@{ Label='brisk'; Display='BRISK' },
    [pscustomobject]@{ Label='kaze'; Display='KAZE' },
    [pscustomobject]@{ Label='orb'; Display='ORB' },
    [pscustomobject]@{ Label='sift'; Display='SIFT' },
    [pscustomobject]@{ Label='surf'; Display='SURF' }
)

$Text = @{
    ReportTitle = '&#28857;&#29305;&#24449;&#27861;&#23545;&#27604;&#27979;&#35797;&#25253;&#21578;'
    Section1 = '1. &#20845;&#31181;&#26041;&#27861;&#30340;&#24635;&#27979;&#35797;&#32467;&#26524;&#21644;&#25104;&#21151;&#29575;'
    Section2 = '12 &#20010;&#29992;&#20363;&#35814;&#32454;&#27979;&#35797;&#32467;&#26524;&#19982;&#36807;&#31243;&#20449;&#24687;&#32479;&#35745;&#34920;'
    Section3 = '&#21487;&#35270;&#21270;&#32467;&#26524;'
    Sample = '&#29992;&#20363;'
    Status = '&#29366;&#24577;'
    Passed = '&#36890;&#36807;'
    ...
    SuccessCount = '&#25104;&#21151;&#25968;'
    SuccessRate = '&#25104;&#21151;&#29575;'
    AvgInlierRatio = '&#24179;&#22343;&#20869;&#28857;&#29575;'
    AvgEdgeIoU = '&#24179;&#22343;&#36793;&#32536; IoU'
    AvgTotalMs = '&#24179;&#22343;&#24635;&#32791;&#26102; ms'
    Method = '&#26041;&#27861;'
    SourceKpFig = '&#28304;&#22270;&#20851;&#38190;&#28857;&#26816;&#27979;&#22270;'
    TargetKpFig = '&#30446;&#26631;&#22270;&#20851;&#38190;&#28857;&#26816;&#27979;&#22270;'
    InlierFig = '&#20869;&#28857;&#21305;&#37197;&#22270;'
    FalseColorFig = '&#20266;&#33394;&#24425;&#37197;&#20934;&#35823;&#24046;&#21472;&#21152;&#22270;'
    MissingSourceKp = '&#26410;&#25214;&#21040;&#28304;&#22270;&#20851;&#38190;&#28857;&#26816;&#27979;&#22270;'
    MissingTargetKp = '&#26410;&#25214;&#21040;&#30446;&#26631;&#22270;&#20851;&#38190;&#28857;&#26816;&#27979;&#22270;'
    MissingInlier = '&#26410;&#25214;&#21040;&#20869;&#28857;&#21305;&#37197;&#22270;'
    MissingFalseColor = '&#26410;&#25214;&#21040;&#20266;&#33394;&#24425;&#37197;&#20934;&#35823;&#24046;&#21472;&#21152;&#22270;'
}
...
function Read-MethodRows {
    ...
    foreach ($row in $rows) {
        ...
        [pscustomobject]@{
            ...
            NumKeypoints1 = $row.num_keypoints_first
            NumKeypoints2 = $row.num_keypoints_second
            NumRawMatches = $row.num_raw_matches
            NumFilteredMatches = $row.num_filtered_matches
            NumInliers = $row.num_inliers
            InlierRatio = $row.inlier_ratio
            ReprojError = $row.mean_reproj_error
            Containment = $row.warp_overlap_containment
            BidirectionalCoverage = $row.warp_bidirectional_coverage
            EdgeIoU = $row.warp_edge_alignment_iou
            NMAD = $row.warp_photometric_error
            MatchMs = $row.t_match_ms
            TotalMs = $row.t_total_ms
        }
    }
}
...
function Build-Report {
    ...
    $overallRows = foreach ($method in $methods) {
        ...
        '<tr><td>{0}</td><td>{1}</td><td>{2} / {1}</td><td>{3}%</td><td>{4}</td><td>{5}</td><td>{6}</td></tr>' -f `
            ...
            $ok,
            (FormatNumber $rate 1),
            (FormatNumber (MeanValue $items 'InlierRatio') 3),
...
<section class="sample-visual">
  ...
  <div class="image-grid">
    $(ImageCellHtml $Text.SourceKpFig $sourceKeypointsImage $Text.MissingSourceKp)
    $(ImageCellHtml $Text.TargetKpFig $targetKeypointsImage $Text.MissingTargetKp)
    $(ImageCellHtml $Text.InlierFig $inlierMatchImage $Text.MissingInlier)
    $(ImageCellHtml $Text.FalseColorFig $falseColorImage $Text.MissingFalseColor)
  </div>
</section>
"@
        }
...
<section class="method-section page-break">
  ...
  <table>
    <thead>
      <tr><th>$($Text.Sample)</th><th>$($Text.Status)</th><th>$($Text.Message)</th><th>$($Text.SourceKp)</th><th>$($Text.TargetKp)</th><th>$($Text.RawMatches)</th><th>$($Text.FilteredMatches)</th><th>$($Text.Inliers)</th><th>$($Text.InlierRatio)</th><th>$($Text.Containment)</th><th>$($Text.BiCoverage)</th><th>$($Text.EdgeIoU)</th><th>$($Text.Nmad)</th><th>$($Text.MatchMs)</th><th>$($Text.TotalMs)</th></tr>
    </thead>
    <tbody>
      $($detailRows -join "`n")
    </tbody>
...
<table>
  <thead>
    <tr><th>$($Text.Method)</th><th>$($Text.CaseCount)</th><th>$($Text.SuccessCount)</th><th>$($Text.SuccessRate)</th><th>$($Text.AvgInlierRatio)</th><th>$($Text.AvgEdgeIoU)</th><th>$($Text.AvgTotalMs)</th></tr>
  </thead>
  <tbody>
    $($overallRows -join "`n")
  </tbody>
...
function Convert-HtmlToPdf {
    ...
    $profile = Join-Path $ProjectRoot 'outputs\compare_keypoint_report\.edge-pdf-profile'
